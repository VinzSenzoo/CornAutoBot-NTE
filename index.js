...
function getAxiosConfig(proxy, additionalHeaders = {}) {
  const headers = {
    ...
    'pragma': 'no-cache',
    'priority': 'u=1, i',
    'referer': 'https://loyalty.usecorn.com/',
...
async function createSignedPayload(privateKey, address, nonce) {
  try {
    ...
    const fullMessage = `loyalty.usecorn.com wants you to sign in with your Ethereum account:\n` +
      `${address}\n\n` +
...
async function fetchNonce(address, proxy, context, refCode = 'MCX7WZUG') {
  const url = 'https://loyalty.usecorn.com/api/auth/csrf';
...
async function executeLogin(privateKey, address, nonce, proxy, context, cookies) {
  const url = 'https://loyalty.usecorn.com/api/auth/callback/credentials';
...
async function retrieveBalance(address, proxy, context, cookies, webId = '8e54cfff-37f1-4d29-b025-8f8aaa1fd331', orgId = 'c798ad76-e2ba-4ed9-9798-4894facd2c2f') {
  const url = `https://loyalty.usecorn.com/api/loyalty/accounts?websiteId=${webId}&organizationId=${orgId}&walletAddress=${address}`;
  ...
  try {
    ...
    if (response.data.data && response.data.data.length > 0) {
      return response.data.data[0].amount || 0;
    } else {
      return 0;
    }
  } catch (error) {
...
async function executeDailyCheckin(address, proxy, context, cookies) {
  const url = 'https://loyalty.usecorn.com/api/loyalty/rules/799c40a7-aff8-4aee-9585-eb88149198d8/complete';
  const config = getAxiosConfig(proxy, {
    'Content-Type': 'application/json',
    'Content-Length': '2',
    'Cookie': cookies.join('; ')
...
async function processAccount(privateKey, index, total, proxy) {
  ...
  try {
    ...
    let currentCookies = [`referral_code=MCX7WZUG`, ...nonceData.setCookie.map(ck => ck.split('; ')[0])];
...
async function run() {
  ...
  console.log(gradient.retro(centerText('✪ BOT CORN AUTO CHECK-IN ✪', terminalWidth)));
  console.log('\n');
